...
def test_mongo_document():
    ...
    t = MongoModelTest(
        num=2, args={"foo": "bar"}, user=ObjectId("666666663333366666666666")
    )
...
def test_versionable_document():
    class InnerModel(BaseModel):
        ...
        string_field: Optional[str] = Field(
            None, description="Another optional string field in inner model"
        )
        number_field: Optional[int] = Field(
            None, description="Another optional number field in inner model"
        )
    ...
    class TestModel(BaseModel):
        ...
        string_list_field: Optional[List[str]] = Field(
            None, description="An optional string list field"
        )
        dict_field: Optional[Dict[str, Any]] = Field(
            None, description="An optional dictionary field"
        )
        base_model_field: Optional[InnerModel] = Field(
            None, description="An optional base model field"
        )
