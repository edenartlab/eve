...
warnings.filterwarnings(
    "ignore",
    message='Field name "schema" in "VersionableBaseModel" shadows an attribute',
)
...
class VersionableBaseModel(BaseModel):
    ...
    def __init__(self, instance: BaseModel = None, **kwargs):
        if instance is not None:
            data = {"schema": type(instance), "initial": instance, "current": instance}
            super().__init__(**data)
        else:
...
def generate_edit_model(model: Type[BaseModel]) -> Type[BaseModel]:
    ...
    for name, field in model.__annotations__.items():
        ...
        if origin in (list, List):
            ...
            if isinstance(item_type, type) and issubclass(item_type, BaseModel):
                ...
                edit_fields[f"add_{name}"] = (
                    Optional[Dict[str, Union[int, item_type]]],
                    None,
                    f"Add {model.__name__} {name} ({field_description})",
                )
                edit_fields[f"edit_{name}"] = (
                    Optional[Dict[str, Union[int, nested_edit_model]]],
                    None,
                    f"Edit {model.__name__} {name} ({field_description})",
                )
                edit_fields[f"remove_{name}"] = (
                    Optional[int],
                    None,
                    f"Remove {model.__name__} {name} ({field_description})",
                )
            else:
                edit_fields[f"add_{name}"] = (
                    Optional[Dict[str, Union[int, item_type]]],
            ...
            if isinstance(value_type, type) and issubclass(value_type, BaseModel):
                ...
                edit_fields[f"add_{name}"] = (
                    Optional[Dict[key_type, value_type]],
                    None,
                    f"Add {model.__name__} {name} ({field_description})",
                )
                edit_fields[f"edit_{name}"] = (
                    Optional[Dict[key_type, nested_edit_model]],
                    None,
                    f"Edit {model.__name__} {name} ({field_description})",
                )
                edit_fields[f"remove_{name}"] = (
    ...
    edit_model = create_model(
        ...
        **{
            key: (value[0], Field(default=value[1], description=value[2]))
...
def parse_schema(schema: dict) -> Tuple[Dict[str, Tuple[Type, Any]], dict]:
    ...
    for field, props in schema.get("parameters", {}).items():
        ...
        fields[field] = (
            type_annotation,
            Field(**field_kwargs, json_schema_extra=json_schema_extra),
        )
