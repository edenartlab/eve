...
def smart_frame_selection(orig_size, target_size):
    ...
    if target_size >= orig_size:
        ...
        if remainder > 0:
            ...
            if orig_size > 2:
                middle_indices = (
                    np.linspace(0, orig_size - 1, remainder).round().astype(int)
                )
        ...
        if target_size == 1:
            ...
            middle_indices = (
                np.linspace(1, orig_size - 2, target_size - 2).round().astype(int)
            )
...
async def handler(context: ToolContext):
    ...
    target_fps = context.args.get("target_fps")
    total_frames = context.args.get("total_frames")
    duration = context.args.get("duration")
    blend_strength = context.args["blend_strength"]
    method = context.args["method"]
    loop_seamless = context.args["loop_seamless"]
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if context.args.get("use_rife", False):
            from .rife import interpolate_sequence

            output = interpolate_sequence(
                images, target_frames, loop_seamless=loop_seamless
            )
        else:
            ...
            if orig_size == 1:
                ...
                if loop_seamless:
                    indices = (
                        np.linspace(0, orig_size, target_frames) % orig_size
                    ).astype(int)
                ...
                if loop_seamless:
                    ...
                    if target_frames > orig_size:
                        frame_indices = (
                            np.linspace(0, target_frames - 1, orig_size)
                            .round()
                            .astype(int)
                        )
        ...
        if has_audio:
            ...
            preserve_pitch = context.args.get("preserve_pitch", True)
            ...
            ffmpeg_command.extend(
                [
                    ...
                    # Video settings
                    "-map",
                    "0:v:0",  # Use video from first input (PNG frames)
                    "-map",
                    "1:a:0",  # Use audio from second input (original video)
                    "-c:v",
                    "h264",  # Video codec
                    "-crf",
                    "23",  # Video quality
