...
class Reel(BaseModel):
    ...
    voiceover: str = Field(
        ...,
        description="The text of the voiceover, if one is not provided by the user.",
    )
...
def write_reel(args: dict, agent: Agent = None):
    ...
    if agent:
        ...
        system_prompt = """You are a critically acclaimed creative writer who writes incredibly captivating and original short-length single-scene reels of 30-60 seconds in length which are widely praised.
        """
...
def write_visual_prompts(reel: Reel, num_clips: int, instructions: str = None):
    ...
    class VisualPrompts(BaseModel):
        ...
        prompts: List[str] = Field(
            ...,
            description="A sequence of visual prompts, containing a content description, and a set of self-similar stylistic modifiers and aesthetic elements, mirroring the style of the original visual prompt.",
        )
...
async def handler(context: ToolContext):
    ...
    if context.args.get("use_voiceover") and reel.voiceover:
        ...
        speech_audio = await elevenlabs.async_run(
            {"text": reel.voiceover, "voice": voice}
        )
    ...
    if context.args.get("use_music"):
        ...
        music_audio = await musicgen.async_run(
            {"prompt": music_prompt, "duration": int(duration)}
        )
        ...
        music_file = utils.download_file(
            music_audio["output"][0]["url"], temp_file.name + ".mp3"
        )
    ...
    async def generate_clip_with_retry(args, prompt, duration, ratio):
        async def _generate():
            ...
            async def generate_video_with_model(
                model_type, prompt, duration, ratio, image_url
            ):
            ...
            for attempt in range(2):  # Try primary, then fallback
                video = await generate_video_with_model(
                    current_model, prompt, duration, ratio, image_url
                )
                ...
                if video and "output" in video and video["output"]:
                    ...
                    error_msg = (
                        video.get("error", "Unknown error") if video else "No response"
                    )
                    ...
                    if attempt == 0 and current_model in fallback_map:
                        ...
                        raise Exception(
                            f"All video generation attempts failed. Last error: {error_msg}"
                        )
        ...
        return await utils.async_exponential_backoff(
            _generate, max_attempts=2, initial_delay=1, max_jitter=1
        )
    ...
    for i in range(num_clips):
        ...
        if use_lora:
            flux_args.update(
                {"use_lora": True, "lora": lora, "lora_strength": lora_strength}
            )

        tasks.append(
            bounded_generate_clip(
                args=flux_args,
                prompt=visual_prompts[i % len(visual_prompts)],
    ...
    return {"output": final_video_url, "intermediate_outputs": {"videos": videos}}
