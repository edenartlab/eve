...
def compute_engagement_score(pm: Dict) -> Tuple[int, str]:
    ...
    engagement_score = (
        pm["retweet_count"] * RETWEET_VALUE + pm["like_count"] * LIKE_VALUE
    )

    if "impression_count" in pm:
        engagement_score = (
            engagement_score + pm["impression_count"] * IMPRESSION_VALUE
        ) / 3
    else:
...
def process_payload(
    ...
    for t in raw.get("data", []):
        ...
        if (
            pm["like_count"] >= min_likes
            and pm["retweet_count"] >= min_retweets
            and fcnt >= min_followers
        ):
            ...
            tweet_url = (
                f"https://x.com/{user.get('username')}/status/{t['id']}"
...
async def handler(args: dict, user: str = None, agent: str = None, session: str = None):
    ...
    tweets = process_payload(
        raw_strict,
        min_likes=MIN_LIKES_HIGH,
        min_retweets=MIN_RETWEETS_HIGH,
        min_followers=MIN_FOLLOWERS_HIGH,
    )
    ...
    if len(tweets) < MIN_KEEP:
        ...
        tweets = process_payload(
            raw_relaxed,
            min_likes=MIN_LIKES_MEDIUM,
            min_retweets=MIN_RETWEETS_MEDIUM,
            min_followers=MIN_FOLLOWERS_MEDIUM,
        )
    ...
    if len(tweets) < MIN_KEEP:
        ...
        relaxed_q = (
            parsed.query.replace("is:verified", " ")
            .replace("-is:retweet", " ")
        ...
        tweets = process_payload(
            raw_very_relaxed,
            min_likes=MIN_LIKES_LOW,
            min_retweets=MIN_RETWEETS_LOW,
            min_followers=MIN_FOLLOWERS_LOW,
        )
