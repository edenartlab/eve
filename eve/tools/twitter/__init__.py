...
class X:
    def __init__(self, agent: Agent):
        ...
        self.bearer_token = agent.secrets[
            "CLIENT_TWITTER_BEARER_TOKEN"
        ].get_secret_value()
        self.consumer_key = agent.secrets[
            "CLIENT_TWITTER_CONSUMER_KEY"
        ].get_secret_value()
        self.consumer_secret = agent.secrets[
            "CLIENT_TWITTER_CONSUMER_SECRET"
        ].get_secret_value()
        self.access_token = agent.secrets[
            "CLIENT_TWITTER_ACCESS_TOKEN"
        ].get_secret_value()
        self.access_token_secret = agent.secrets[
            "CLIENT_TWITTER_ACCESS_TOKEN_SECRET"
        ].get_secret_value()
    ...
    def _make_request(self, method, url, retries=1, oauth=True, **kwargs):
        ...
        try:
            if method.lower() == "get":
                response = (
                    self.oauth.get(url, **kwargs)
                    if oauth
                    else requests.get(url, **kwargs)
                )
            else:
                response = (
    ...
    def post(self, tweet_text, media_urls=None, reply_to_tweet_id=None):
        ...
        for media_url in media_urls:
            ...
            if not media_id:
                logging.error(
                    "Media upload was unsuccessful. Cannot post tweet with media."
                )
                return
        ...
        response = self._make_request(
            "post", "https://api.twitter.com/2/tweets", json=payload
        )
