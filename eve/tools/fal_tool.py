...
class FalTool(Tool):
    ...
    def _process_result(self, result, task):
        ...
        if not isinstance(output_path, list):
             logger.error(f"Invalid output_schema format for tool {self.name}. Expected list, got {type(output_path)}.")
        ...
        if raw_output_value is None:
            logger.error(f"Could not extract value using path {output_path} from result for tool {self.name}")
            output_urls = []
            ...
            if not output_urls:
                logger.warning(f"Output path {output_path} points to a list, but couldn't extract string URLs directly.")
                 # Attempt basic extraction if list contains dicts with 'url' key - simplistic fallback
                output_urls = [item['url'] for item in raw_output_value if isinstance(item, dict) and 'url' in item]
        else:
            logger.warning(f"Unexpected type for output value at path {output_path}: {type(raw_output_value)}")
            output_urls = []

        if not output_urls:
             logger.error(f"No output URLs extracted using path {output_path} for tool {self.name}, task {task.id}. Returning raw result.")
        ...
        for r, res_item in enumerate(final_result_structure):
            for o, output_data in enumerate(res_item["output"]):
                 ...
                 if not isinstance(output_data, dict):
                     logger.warning(f"Skipping creation object for non-dict output: {output_data}")
                     continue

                 name = task.args.get("prompt", task.args.get("text_input", "")) # Try getting prompt/text_input
                 creation = Creation(
                     user=task.user,
                     agent=task.agent,
                     task=task.id,
