...
class ReplicateTool(Tool):
    ...
    async def async_run(self, context: ToolContext):
        ...
        if self.version:
            ...
            await asyncio.to_thread(prediction.wait)
            ...
            args = self._format_args_for_replicate(args)
            output = await asyncio.to_thread(replicate.run, replicate_model, input=args)
    ...
    async def async_cancel(self, task: Task):
        prediction = await asyncio.to_thread(replicate.predictions.get, task.handler_id)
