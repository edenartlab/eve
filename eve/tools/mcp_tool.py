...
class MCPTool(Tool):
    ...
    mcp_tool_name: str = Field(..., description="Name of the tool to call on the MCP server")
    mcp_env_params: Optional[Dict[str, str]] = Field(None, description="Environment variable mappings for query params")
    ...
    async def _call_mcp_tool(self, args: Dict[str, Any]) -> str:
        ...
        print(f"MCP Tool Debug - URL: {url}")
        print(f"MCP Tool Debug - Tool name: {self.mcp_tool_name}")
        print(f"MCP Tool Debug - Args: {formatted_args}")
        ...
        async with streamablehttp_client(url) as (read_stream, write_stream, get_session_id):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                print(f"MCP Tool Debug - Raw result type: {type(result)}")
                print(f"MCP Tool Debug - Raw result: {result}")
                
                if hasattr(result, 'content') and result.content:
                    print(f"MCP Tool Debug - Content items count: {len(result.content)}")
                    content_parts = []
                    for i, item in enumerate(result.content):
                        print(f"MCP Tool Debug - Content item {i} type: {type(item)}")
                        print(f"MCP Tool Debug - Content item {i}: {item}")
                        if hasattr(item, 'text'):
                            content_parts.append(item.text)
                        else:
                            content_parts.append(str(item))
                    final_result = "\n".join(content_parts) if content_parts else str(result)
                    print(f"MCP Tool Debug - Final result: {final_result}")
                    return final_result
                
                print(f"MCP Tool Debug - No content, returning str(result)")
                return str(result)
    ...
    def _format_args(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in args.items():
            ...
            if param_type == "integer":
                formatted[target_key] = int(value) if not isinstance(value, int) else value
            elif param_type == "number":
                formatted[target_key] = float(value) if not isinstance(value, (int, float)) else value
            elif param_type == "boolean":
                formatted[target_key] = bool(value) if not isinstance(value, bool) else value
            elif param_type == "array":
                formatted[target_key] = list(value) if not isinstance(value, list) else value
            else:
                formatted[target_key] = str(value) if not isinstance(value, str) else value
