...
async def handler(context: ToolContext):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if not creator_name:
            ...
            try:
                creators_response = await _api_request(
                    client, "POST", "/api/ads/list-creators"
                )
        ...
        submit_response = await _api_request(client, "POST", "/api/ads/submit", payload)
        ...
        for attempt in range(max_attempts):
            ...
            status_response = await _api_request(
                client, "POST", "/api/ads/poll", status_payload
            )
