...
@handle_errors
async def handle_google_calendar_auth_url(
    agent_id: str, user_id: str, redirect_uri: str
) -> dict:
    """
    Generate Google OAuth authorization URL.

    The state parameter encodes agent_id, user_id, and redirect_uri
    so we can route back to the correct place after OAuth.

    Google will redirect to the provided redirect_uri (Next.js callback),
    which then calls our Python callback endpoint to exchange the code.
    """
    from eve.agent.deployments.google_calendar import create_oauth_flow

    # Verify agent exists
    agent = Agent.from_mongo(ObjectId(agent_id))
    if not agent:
        raise APIError(f"Agent not found: {agent_id}", status_code=404)

    # Build state parameter (will be returned by Google callback)
    state = json.dumps(
        {"agent_id": agent_id, "user_id": user_id, "redirect_uri": redirect_uri}
    )

    # Use the redirect_uri passed by the frontend (Next.js callback URL)
    # Google will redirect there, then Next.js calls our Python callback
    flow = create_oauth_flow(redirect_uri=redirect_uri)

    # Generate authorization URL
    # access_type='offline' ensures we get a refresh_token
    # prompt='consent' forces consent screen to ensure refresh_token
...
@handle_errors
async def handle_google_calendar_callback(code: str, state: str) -> dict:
    """
    Handle OAuth callback from Google.

    Exchanges the authorization code for tokens, fetches user info,
    and returns the credentials along with available calendars.

    Called by the frontend (Next.js) callback handler, not directly by Google.
    """
    ...
    try:
        ...
        frontend_redirect_uri = state_data["redirect_uri"]
    except (json.JSONDecodeError, KeyError) as e:
        raise APIError(f"Invalid state parameter: {e}", status_code=400)

    # Get the OAuth callback URL (must match what was used in auth URL)
    # This is the Next.js callback URL, not the Python backend
    # The redirect_uri in state is where Google redirected to
    oauth_callback_uri = frontend_redirect_uri

    # Create OAuth flow and exchange code
    flow = create_oauth_flow(redirect_uri=oauth_callback_uri)
    ...
    try:
        ...
        raise APIError(f"Failed to authenticate with Google: {e}", status_code=400)

    if not credentials.refresh_token:
        logger.warning("No refresh token received - user may have already authorized")
        raise APIError(
            "No refresh token received. Please revoke access in Google Account settings and try again.",
            status_code=400,
        )
    ...
    gc_secrets = secrets_from_credentials(
        credentials=credentials, google_email=google_email, google_user_id=google_user_id
    )

    # Fetch available calendars
    try:
        calendars = await list_user_calendars(gc_secrets)
        calendar_list = [
            {
                "id": cal.get("id"),
    ...
    # Return credentials and calendars to frontend
    # The frontend will then call the deployment create/update endpoint
    return {
        "success": True,
        "google_email": google_email,
        "credentials": gc_secrets.model_dump(),
...
@handle_errors
async def handle_google_calendar_calendars(agent_id: str) -> dict:
    """
    List calendars for an existing Google Calendar deployment.

    This is used when the user wants to change which calendar is selected.
    """
    from eve.agent.deployments.google_calendar import list_user_calendars
    ...
    # Find existing Google Calendar deployment
    deployment_doc = Deployment.get_collection().find_one(
        {"agent": agent.id, "platform": ClientType.GOOGLE_CALENDAR.value}
    )

    if not deployment_doc:
        raise APIError(
            "No Google Calendar deployment found. Connect Google Calendar first.",
            status_code=404,
        )

    deployment = Deployment.from_mongo(deployment_doc["_id"])

    if not deployment.secrets or not deployment.secrets.google_calendar:
        raise APIError(
            "Google Calendar credentials not found. Reconnect your account.",
            status_code=400,
        )

    # Fetch calendars using stored credentials
    calendars = await list_user_calendars(deployment.secrets.google_calendar)
