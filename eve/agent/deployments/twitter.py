...
async def process_twitter_tweet(
    ...
    try:
        ...
        deployment = Deployment.from_mongo(deployment_id)  # type: ignore[arg-type]
        ...
        # Extract tweet info
        tweet = tweet_data["data"] if "data" in tweet_data else tweet_data
        if not isinstance(tweet, dict):
            raise Exception("No tweet data found")
        includes = tweet_data.get("includes") or {}
        ...
        if not session:
            try:
                linked_tweet = Tweet.find_one(  # type: ignore[attr-defined]
        ...
        if not session:
            # Title will be generated by async_title_session hook on first message
            ...
            if conversation_id and conversation_id != tweet_id:
                ...
                try:
                    ...
                    for prev_tweet in prev_tweets:
                        ...
                        message = ChatMessage(  # type: ignore[call-arg]
                            createdAt=created_at,
                            session=[session.id],
                            channel=Channel(  # type: ignore[call-arg]
                        ...
                        if role == "user" and tweet_user.id:
        ...
        prompt_context = PromptSessionContext(
            ...
            message=ChatMessageRequestInput(
                channel=Channel(type="twitter", key=tweet_id, url=twitter_url),  # type: ignore[call-arg]
                content=text,
        ...
        try:
            existing_tweet = Tweet.find_one({"_id": tweet_id})  # type: ignore[attr-defined]
            if existing_tweet:
