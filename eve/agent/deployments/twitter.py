...
async def fetch_tweet_ancestry(tweet_id: str, conversation_id: str, twitter_api) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        current_tweet_time = None
        for i, tweet in enumerate(tweets):
            if tweet["id"] == tweet_id:
                current_tweet_time = tweet.get("created_at")
...
async def unpack_tweet(tweet: Dict[str, Any], includes: Dict[str, Any]):
    ...
    return tweet_id, author_id, author_username, author_data, text, media_urls, created_at
        ...
        try:
            ...
            if conversation_id and conversation_id != tweet_id:
                ...
                try:
                    ...
                    prev_tweets_data = await fetch_tweet_ancestry(tweet_id, conversation_id, twitter_api)
                    ...
                    for prev_tweet in prev_tweets:
                        tweet_id_, author_id_, author_username_, author_data_, text_, media_urls_, created_at_ = await unpack_tweet(prev_tweet, prev_includes)

                        # Upload media to S3
                        media_urls_ = upload_media_to_s3(media_urls_)

                        # Parse created_at
                        created_at = datetime.fromisoformat(created_at_.replace("Z", "+00:00")) if created_at_ else datetime.now(timezone.utc)
...
async def poll_twitter_gateway(local_mode: bool = False):
    ...
    try:
        ...
        eve = Agent.load("eve")
        auth_deployment = Deployment.find_one({
            "platform": "twitter",
            "agent": eve.id
        })
        ...
        active_twitter_deployments = [
            d for d in Deployment.find({"platform": "twitter"})
            if (d.config and d.config.twitter and
                d.config.twitter.enable_tweet and
                d.config.twitter.username)
        ]
        ...
        tracked_usernames = [d.config.twitter.username for d in active_twitter_deployments]
        logger.info(f"Tracking {len(tracked_usernames)} Twitter accounts: {tracked_usernames}")

        # Build deployment map for quick lookup (username -> deployment)
        deployment_map = {
            d.config.twitter.username: d
        ...
        while page < MAX_PAGES_PER_QUERY:
            try:
                ...
                try:
                    ...
                    logger.error(f"This is likely an auth error. Check token validity.")
                    logger.error(f"Try running: DB={db.upper()} python test_twitter_api.py")
                    raise
                ...
                if "errors" in response_data:
                    logger.error(f"Twitter API returned errors: {response_data['errors']}")
                    break
                ...
                for tweet in tweets:
                    ...
                    if author_id in agent_twitter_ids:
                        logger.info(f"Skipping tweet from agent itself (author_id: {author_id})")
                        continue
                    ...
                    if "mentions" in entities:
                        mentioned_usernames = [m["username"] for m in entities["mentions"]]
                    ...
                    logger.info(f"Found matching deployment {matching_deployment.id} via {match_reason} for tweet {tweet_id}")
                    ...
                    if existing_tweet and hasattr(existing_tweet, 'processed') and existing_tweet.processed:
                    ...
                    try:
                        if local_mode:
                            ...
                            logger.info(f"Processing tweet {tweet_id} locally (synchronous)")
                            result = await process_twitter_tweet(tweet_id, tweet_data, str(matching_deployment.id))
                            ...
                            func.spawn(tweet_id, tweet_data, str(matching_deployment.id))
