...
def calculate_dynamic_limits(
    base_memory_types: dict, char_counts_by_source: dict, log_power: float = 4.0
) -> dict:
    """
    ...
    multiplier = max(
        multiplier, 0.5
    )  # Cap at 0.5x to prevent underflow memory formation
    multiplier = min(
        multiplier, 2.0
    )  # Cap at 2.0x to prevent excessive memory formation
...
class SessionMemory(Document):
    ...
    def ensure_indexes(cls):
        ...
        collection.create_index(
            [("agent_id", 1), ("memory_type", 1), ("related_users", 1)],
            name="directive_lookup_idx",
            background=True,
        )
        ...
        collection.create_index(
            [("source_session_id", 1), ("memory_type", 1), ("createdAt", -1)],
            name="episode_memories_idx",
            background=True,
        )
...
class AgentMemory(Document):
    ...
    is_active: Optional[bool] = (
        True  # Can be set to False to disable the shard without deleting it
    )
    agent_owner: Optional[ObjectId] = None
    ...
    max_agent_memories_before_consolidation: Optional[int] = (
        MAX_AGENT_MEMORIES_BEFORE_CONSOLIDATION
    )
    ...
    def ensure_indexes(cls):
        ...
        collection.create_index(
            ...
            name="agent_id_is_active_idx",
            background=True,
        )

        # This optimizes: AgentMemory.find_one({"agent_id": agent_id, "is_active": True}, sort=[("last_updated_at", -1)])
        collection.create_index(
            [("agent_id", 1), ("is_active", 1), ("last_updated_at", -1)],
            name="agent_memory_freshness_idx",
            background=True,
        )
...
def _format_memories_with_age(memories: List[SessionMemory]) -> str:
    ...
    for memory in memories:
        if memory.createdAt:
            ...
            created_at = (
                memory.createdAt.replace(tzinfo=timezone.utc)
                if memory.createdAt.tzinfo is None
                else memory.createdAt
            )
...
def _get_recent_messages(
    session_messages: List[ChatMessage], last_memory_message_id: ObjectId
) -> List[ChatMessage]:
