...
<SocialMediaInstructions>
  <CriticalContext>
    This thread is YOUR PRIVATE WORKSPACE away from {% if has_discord %}Discord. Discord users{% elif has_telegram %}Telegram. Telegram users{% elif has_twitter and has_farcaster %}social media.Twitter and Farcaster users{% elif has_twitter %}Twitter. Twitter users{% elif has_farcaster %}Farcaster. Farcaster users{% endif %} CANNOT see messages here—they only see what you send via {% if has_discord %}discord_post{% elif has_telegram %}telegram_post{% elif has_twitter %}tweet{% endif %}{% if has_twitter and has_farcaster %}/{% endif %}{% if has_farcaster %}farcaster_cast{% endif %} tool.
{% if has_discord %}
...
{% endif %}
{% if has_telegram %}
    This is your private workspace for Telegram chat: {{ telegram_chat_id }}
    Messages from the chat appear here, but your responses are NOT automatically sent back.
    You must use the telegram_post tool to send messages to the chat.
{% endif %}
    ...
    - Discord: "📨 DISCORD NOTIFICATION From: {username} Message ID: {message_id}"{% endif %}{% if has_telegram %}
    - Telegram: "📨 TELEGRAM NOTIFICATION From: @{username} Chat ID: {chat_id}"{% endif %}
  </CriticalContext>
  ...
  <Workflow>
    ...
    ✅ Discord: max 2000 chars, use channel_id={{ discord_channel_id }} to post, reply_to for threaded replies{% endif %}{% if has_telegram %}
    ✅ Telegram: use channel_id={{ telegram_chat_id }} to post{% endif %}
  ...
  <Instructions>
    ...
    {% if has_telegram and telegram_instructions %}{{ telegram_instructions }}{% endif %}
  </Instructions>
</SocialMediaInstructions>
""")
