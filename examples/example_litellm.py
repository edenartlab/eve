...
def get_current_weather(location, unit="fahrenheit"):
    ...
    city_weather = weather_data.get(
        location, {"temperature": 70, "condition": "unknown"}
    )
    return json.dumps(
        {
            "location": location,
...
def main():
    ...
    if (
        hasattr(response_message, "thinking_blocks")
        and response_message.thinking_blocks
    ):
        print("\n=== THINKING BLOCKS ===")
    ...
    if tool_calls:
        ...
        if (
            hasattr(second_message, "reasoning_content")
            and second_message.reasoning_content
        ):
        ...
        if (
            hasattr(second_message, "thinking_blocks")
            and second_message.thinking_blocks
        ):
