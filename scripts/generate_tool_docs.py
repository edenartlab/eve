...
def generate_param_field(name: str, param: Dict[str, Any], is_required: bool = False) -> str:
...
def main():
    parser = argparse.ArgumentParser(description='Generate Mintlify documentation for Eden tools')
    parser.add_argument('tools', nargs='*', help='List of tool names to generate docs for')
    parser.add_argument('--all', action='store_true', help='Generate documentation for all tools')
    parser.add_argument('--eve-dir', default='../eve', help='Path to eve repository (default: ../eve)')
    parser.add_argument('--docs-dir', default='../mintlify-docs', help='Path to mintlify-docs repository (default: ../mintlify-docs)')
    ...
    if args.tools:
        ...
        print("Error: No tools specified. Use --all to generate all tools or provide tool names.", file=sys.stderr)
        sys.exit(1)
        ...
        try:
            ...
            for output_type, display_name in type_order:
                if output_type in tools_by_type:
                    groups.append({
                        'group': display_name,
                        'pages': [f'tools/{name}' for name in tools_by_type[output_type]]
                    })
            
            # Add any remaining output types not in our predefined order
            remaining_types = set(tools_by_type.keys()) - set([t[0] for t in type_order])
            ...
            if not tools_tab:
                ...
                api_index = next((i for i, tab in enumerate(docs_config['navigation']['tabs']) 
                                 if tab.get('tab') == 'API reference'), len(docs_config['navigation']['tabs']))
