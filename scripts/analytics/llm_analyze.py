...
def create_chunk_from_threads(
    available_threads: List[str], target_size: int
) -> Tuple[str, List[str]]:
...
def chunk_conversations(
    ...
    while available_threads:
        chunk, available_threads = create_chunk_from_threads(
            available_threads, chunk_size
        )
...
Format your response in a clear, structured way that can be easily parsed for the second phase of analysis."""
    ...
    try:
        ...
        print(
            f"Loaded {len(text)} characters from file. Chunking into aproximately {total_chunks} chunks of {chunk_size} characters."
        )

        # Process text in chunks
        for i, (chunk, remaining_threads) in enumerate(
            chunk_conversations(text, chunk_size), 1
        ):
